...
def compute_cost_with_components(coeffs, basis, x0, y0, fx, fy, sigma1, sigma2, dt, 
    ...
    # This directly addresses the Chebyshev boundary oscillation problem
...
def optimize_cell_fate_control(params, verbose=True, convergence_window=50, 
    ...
    if verbose:
        ...
        print(f"  Terminal penalty coefficient: α = {alpha_terminal}")
    ...
    beta = params.get('beta', 0.1)  # Updated default for better performance
    alpha_terminal = params.get('alpha_terminal', 2.0)  # Optimal spike prevention value
    ...
    terminal_penalties = []
    distances = []
    control_spikes = []  # Track maximum control values to monitor spikes
    ...
    for epoch in range(max_epochs):
        ...
        if epoch >= convergence_window:
            ...
            if start_loss > 0:  # safeguard against dividing by 0 (error was popping up based on this as well)
...
def comprehensive_graph(results, target_x, target_y, save_name):
    ...
    ax2.scatter(t_vec[-1], results['U1'][-1], color='blue', s=100, 
               label=f'U₁(T) = {results["U1"][-1]:.4f}', zorder=5)
    ax2.scatter(t_vec[-1], results['U2'][-1], color='orange', s=100, 
               label=f'U₂(T) = {results["U2"][-1]:.4f}', zorder=5)
    ...
    ax8.plot(t_vec[terminal_window], results['U1'][terminal_window], 'b-', linewidth=2, label='U₁(t)')
    ax8.plot(t_vec[terminal_window], results['U2'][terminal_window], 'r-', linewidth=2, label='U₂(t)')
    ...
    summary_text = (
        f"SPIKE OPTIMIZATION SUMMARY\n"
        f"{'='*25}\n"
        f"Ensemble size: {results['ensemble_size']}\n"
        f"Chebyshev order: {results['chebyshev_order']}\n"
        f"Terminal penalty: α = {results.get('alpha_terminal', 'N/A')}\n"
    ...
    x_roots = find_x_steady_states_y0(results['a'], results['theta1'])
    for x_root in x_roots:
        ax11.scatter(x_root, 0, color='orange', s=100, marker='s', alpha=0.9, 
    ...
    ax12.text(0.1, 0.3, 'U₁(T) = 0  and  U₂(T) = 0', transform=ax12.transAxes, 
    ...
    plt.close()
    print(f"Spike-optimized comprehensive graphs saved: {save_name}")
...
def sweep_lambda(base_params, lambda_values, save_name="S1_lambda_parameter_sweep", verbose=False):
    ...
    axes = axes.flatten()
    fig.suptitle('Effect of Regularization Parameter λ on Spike-Free Control Performance', 
    ...
    table_data = []
    headers = ['λ', 'Success Rate (%)', 'Max |U|', 'Terminal |U|', 'Final Loss']
...
def sweep_beta(base_params, beta_values, save_name="S2_beta_parameter_sweep", verbose=False):
    ...
    axes = axes.flatten()
    fig.suptitle('Effect of Exponential Weighting Parameter β on Spike-Free Control', fontsize=16, fontweight='bold')
...
def main():
    ...
    # Spike-optimized steady state analysis
    ...
    # Spike-optimized Potential Landscape Comparison 
    print("\nSPIKE-OPTIMIZED POTENTIAL LANDSCAPE COMPARISON")
